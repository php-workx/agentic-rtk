...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    runner::run_filtered(
        ...
        |raw| {
            ...
            // Note: compact_ls has its own internal fallback for parse failures; this
            // caller-side check is a secondary safety net for edge cases the internal
            // fallback doesn't cover.
            let has_real_content = raw
                .lines()
                .any(|l| !l.starts_with("total ") && !l.is_empty() && !is_dotdir(l));
            if parsed_count == 0 && has_real_content && (entries.is_empty() || entries == "(empty)\n") {
...
///
/// Returns an empty string if the input does not look like a permission field.
...
fn perms_to_octal(perms: &str) -> String {
    ...
    let owner = (if b[1] == b'r' { 4 } else { 0 })
        + (if b[2] == b'w' { 2 } else { 0 })
        + (if owner_x { 1 } else { 0 });
    let group = (if b[4] == b'r' { 4 } else { 0 })
        + (if b[5] == b'w' { 2 } else { 0 })
        + (if group_x { 1 } else { 0 });
    let other = (if b[7] == b'r' { 4 } else { 0 })
        + (if b[8] == b'w' { 2 } else { 0 })
        + (if other_x { 1 } else { 0 });
    ...
    let special = (if setuid { 4 } else { 0 })
        + (if setgid { 2 } else { 0 })
        + (if sticky { 1 } else { 0 });
...
fn compact_ls(raw: &str, show_all: bool, show_long: bool) -> (String, String, usize) {
    ...
    let mut dirs: Vec<(String, String)> = Vec::new(); // (name, octal_perms)
    ...
    for line in raw.lines() {
        ...
        // Skip . and .. entries before parse tracking — they are intentionally
        // filtered, not parse failures, so they must not pollute the fallback
        // counter. This ensures a directory containing only . and .. is reported
        // as "(empty)" rather than triggering the raw-output fallback.
        if is_dotdir(line) {
            dotdirs += 1;
            continue;
        }
        ...
        let octal = perms_to_octal(&perms);
    ...
    if parse_attempts > 0 && parse_failures == parse_attempts {
        if show_all {
            return (raw.to_string(), String::new(), 0);
        }
...
mod tests {
    ...
    fn test_compact_basic() {
        ...
        let (entries, _summary, _) = compact_ls(input, false, false);
    ...
    fn test_compact_show_all() {
        let input = "total 8\n\
                     drwxr-xr-x  2 user  staff  64 Jan  1 12:00 .git\n\
                     drwxr-xr-x  2 user  staff  64 Jan  1 12:00 src\n";
        let (entries, _summary, _) = compact_ls(input, true, false);
        assert!(entries.contains(".git/"));
        assert!(entries.contains("src/"));
    }
    ...
    fn test_compact_empty() {
        let input = "total 0\n";
        let (entries, summary, _) = compact_ls(input, false, false);
    ...
    fn test_compact_empty_chinese_locale() {
        let input = "total 8\n\
                     drwxr-xr-x  2 user user  4096  1\u{6708}  1 12:00 .\n\
                     drwxr-xr-x  16 user user 20480  1\u{6708}  1 12:00 ..\n";
        // . and .. are identified as dotdirs (ends with '.' / '..') and skipped
        // before parse tracking, so the internal fallback doesn't trigger.
    ...
    fn test_compact_summary() {
        ...
        let (_entries, summary, _) = compact_ls(input, false, false);
    ...
    fn test_parse_ls_line_basic() {
        let (ft, perms, size, name) = parse_ls_line(
    ...
    fn test_parse_ls_line_multiline_group() {
        let (ft, _, size, name) = parse_ls_line(
            "-rw-r--r--  1 fjeanne utilisa. du domaine 0 Mar 31 16:18 empty.txt",
        )
        .unwrap();
        assert_eq!(ft, '-');
    ...
    fn test_compact_localized_output_falls_back_to_raw() {
        ...
        let (entries, _summary, _) = compact_ls(italian_input, false, false);
        assert_eq!(
    ...
    fn test_perms_to_octal_common() {
        ...
        assert_eq!(perms_to_octal("drwxr-xr-x"), "755");
        assert_eq!(perms_to_octal("-rw-------"), "600");
        assert_eq!(perms_to_octal("-rwxrwxrwx"), "777");
        assert_eq!(perms_to_octal("----------"), "000");
        assert_eq!(perms_to_octal("lrwxr-xr-x"), "755");
    }

    #[test]
    fn test_perms_to_octal_special_bits() {
        // setuid + 755 -> 4755
        assert_eq!(perms_to_octal("-rwsr-xr-x"), "4755");
        // setuid without execute -> 4644
        assert_eq!(perms_to_octal("-rwSr--r--"), "4644");
        // setgid + 755 -> 2755
        assert_eq!(perms_to_octal("-rwxr-sr-x"), "2755");
        // sticky bit on /tmp-style dir -> 1777
        assert_eq!(perms_to_octal("drwxrwxrwt"), "1777");
        // setuid + setgid + sticky
        assert_eq!(perms_to_octal("-rwsrwsrwt"), "7777");
    }

    #[test]
    fn test_perms_to_octal_garbage() {
        assert_eq!(perms_to_octal(""), "");
        assert_eq!(perms_to_octal("short"), "");
    }
    ...
    fn test_compact_long_format_includes_octal() {
        ...
        let (entries, _summary, _) = compact_ls(input, false, true);
        assert!(
    ...
    fn test_compact_chinese_locale_fallback() {
        // Chinese-locale ls output with real files that can't be parsed.
        // The internal fallback returns filtered raw output instead of
        // empty output, preserving data visibility.
        let input = "total 8\n\
                      drwxr-xr-x  2 user staff  64  1\u{6708}  1 12:00 src\n\
                      -rw-r--r--  1 user staff 1234  1\u{6708}  1 12:00 main.rs\n";
        let (entries, summary, parsed_count) = compact_ls(input, false, false);
        assert_eq!(parsed_count, 0);
        // Internal fallback returns filtered raw output (non-empty) since
        // all non-dotdir lines failed to parse (Chinese locale dates).
        // This is better than returning empty output which would hide data.
